...
class MascotHitsCSVParser
	...
	# filter by cutoffs and modification
	def highest_from_cutoff_scored_hit_for_prot(protein)
		...
		score = 0
		hits.each do |hit|
			if hit.pep_var_mod.include? "Phospho"
	...
	def create_pep_index()
		...
		@filehandle.each do |line|
			...
			if in_protein_hits_table == true
				unique_peptides[hit.pep_seq] = nil
				# Phospho (ST)
				# 2 Phospho (ST)
				# Oxidation (M); Phospho (ST)
				# Oxidation (M); 2 Phospho (ST)
				# Oxidation (M); 3 Phospho (ST)
				# Phospho (Y)
				# Phospho (ST); Phospho (Y)
				# 2 Phospho (ST); Phospho (Y) 
				# 5 Phospho (ST)
				# 2 Phospho (ST); 2 Phospho (Y)
				# 2 Oxidation (M); 3 Phospho (ST)
				# 3 Phospho (ST)
				# Oxidation (M); 4 Phospho (ST)
				puts hit.pep_var_mod
				if (hit.pep_var_mod.include? "Phospho") && (hit.pep_score.to_f >= @pep_score_cutoff) && (hit.pep_expect.to_f < @cutoff) && (hit.pep_rank.to_i == 1)
					unique_phospho_peptides[hit.pep_seq] = nil
		...
		@filehandle.rewind
		puts "PHOSPHO UNIQUE PEPTIDES = #{unique_phospho_peptides.length}"
		puts "TOTAL UNIQUE PEPTIDES = #{unique_peptides.length}"
	end
	...
	def line_parse(line)
		...
		return ProteinHit.new(prot_hit_num, prot_acc, prot_desc, prot_score, prot_mass, prot_matches, prot_matches_sig, prot_sequences, prot_sequences_sig, prot_len, pep_query, pep_rank, pep_isbold, pep_isunique, pep_exp_mz, pep_exp_mr, pep_exp_z, pep_calc_mr, pep_delta, pep_start, pep_end, pep_miss, pep_score, pep_expect, pep_res_before, pep_seq, pep_res_after, pep_var_mod, pep_var_mod_pos, pep_num_match, pep_scan_title, @filename)
	end
